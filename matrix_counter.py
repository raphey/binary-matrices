...
# under permutation of rows, permutation of columns, inversion of rows, and inversion of columns.
def enumerate_matrices(n, verbose=False):
    ...
    part_symmetry_lookup = {}               # Dict used to avoid duplicating work
    ...
    final_count = grand_total // ((factorial(n) ** 2) * (2 ** (2 * n - 1)))     # Denominator is the size of the group
    print("\nTotal number of distinct %s by %s binary matrices: %s" % (n, n, final_count))
    print("(Distinct under row/col permutation and row/col inversion)")
    print("\nElapsed time: %.2f s" % (time.time() - time0))


enumerate_matrices(6)

# N=5, 39, in 0.09 s
# N=6, 388, in 0.17 s
# N=7, 8102, in 0.28 s
# N=8, 656108, in 0.60 s
# N=9, 199727714 in 2.0 s
# N=10, 224693292768 in 9.6 s
# N=11, 893966897828288 in 41 s
# N=12, 12397352268917562436 in 2.8 m